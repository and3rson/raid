...
`a` - auth-пакет, містить результат авторизації. Дані пакета містять текст `ok`, `timeout` або `wrong_api_key`.

`p` - ping-пакет, надсилається сервером кожні 15 секунд. Дані містять рандомне число в діапазоні [0;10000).

`s` - state-пакет, містить інформацію про зміну статусу тривоги в деякій області. Дані пакета містять номер області та статус тривоги. Наприклад, при активації тривоги в Львівській області міститиме текст `12=1`.
